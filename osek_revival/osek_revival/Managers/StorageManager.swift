...
final class StorageManager{
    ...
    public func downloadUrlForProfilePicture(path: String, completion: @escaping(URL?) -> Void){
        container.reference(withPath: path).downloadURL { url, error in
            completion(url)
        }
    }
    ...
    public func uploadPostHeaderImage(
        ...
        completion: @escaping(Bool) -> Void){
            let path = email.replacingOccurrences(of: "@", with: "_").replacingOccurrences(of: ".", with: "_")
            guard let pngData = image.pngData() else {return}
            container.reference(withPath: "post_images/\(path)/\(postId)").putData(pngData, metadata: nil) { metadata, error in
                guard metadata != nil, error == nil else {
                    completion(false)
                    return
                }
    ...
    public func downloadUrlForPostHeader(email: String,
        ...
        container.reference(withPath: "post_images/\(emailComponent)/\(postId)")
